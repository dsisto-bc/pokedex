...
const getPokemonUrls = async (pokemonPerPage: number): Promise<string[][]> => {
	try {
		let res = await fetch(`${POKEAPI_BASE_URL}/pokemon?limit=2000`);

		if (res.status < 400) {
			const pokemonList = await res.json();
			const paginatedPokemonUrlList = paginatePokemonUrls(pokemonList.results, pokemonPerPage);
			...
			throw new Error(`Error getting pokemons from server. status: ${res.status}, Error: ${res.statusText}`);
		}
	} catch (error) {
