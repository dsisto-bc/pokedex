...
const formatPokemon = (apiResponse): Pokemon => {
	const formattedPokemon = {
		...
		attributes: [
			{ height: apiResponse.height as number },
			{ weight: apiResponse.weight as number },
		],
		types: apiResponse.types.map(
		...
		stats: apiResponse.stats.reduce(
			...
			{
				[apiResponse.stats[0].stat.name]:
					apiResponse.stats[0].base_stat,
			}
		) as Stat[],
		moves: apiResponse.moves.map(
			(apiMove) => apiMove.move.name
		) as string[],
